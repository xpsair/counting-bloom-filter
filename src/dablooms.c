...
/*
  *  If should_positive == 0, then the element is to be removed.
  */
...
/*
  ...
  /*
    *  function: just enlarge the size and copy the original data of a bitmap
    *
    */
    ...
    */
    ...
    if (bitmap->array != NULL) {
        ...
        //bitmap->array = mremap(bitmap->array, old_size, new_size, MREMAP_MAYMOVE);
   	  bitmap->array = realloc(bitmap->array, new_size);
...
/*
  *  Every 4-bit inside a byte as a counter.
  */
int bitmap_increment(bitmap_t *bitmap, unsigned int index, long offset)
{
    long access = index / 2 + offset; //by xp: ???
    uint8_t temp;
...
void hash_func(counting_bloom_t *bloom, const char *key, size_t key_len, uint32_t *hashes)
{
    ...
    for (i = 0; i < bloom->nfuncs; i++) {
        hashes[i] = (h1 + i * h2) % bloom->counts_per_func; //by xp: get the corresponding hash value for each hash func.
    }
}
...
counting_bloom_t *counting_bloom_init(unsigned int capacity, double error_rate, long offset)
{
    ...
    bloom->counting_stats.false_negatives = 0;
    bloom->counting_stats.false_positives = 0;
    bloom->counting_stats.true_negatives = 0;
    bloom->counting_stats.true_positives = 0;
    bloom->bitmap = NULL;
    bloom->capacity = capacity;
    bloom->error_rate = error_rate;
    bloom->offset = offset + sizeof(counting_bloom_header_t); //by xp: because all filters are sorted
    bloom->nfuncs = (int) ceil(log(1 / error_rate) / log(2)); //by xp: return the smallest integral value that is not less than x
    ...
    bloom->num_bytes = ((bloom->size + 1) / 2) + sizeof(counting_bloom_header_t); //by xp: the size of (the header + the bitmap) of this filter
...
int counting_bloom_add(counting_bloom_t *bloom, const char *s, size_t len)
{
    ...
    for (i = 0; i < bloom->nfuncs; i++) {
        ...
        index = hashes[i] + offset; //by xp: offset inside the corresponding range of a hash func
...
int counting_bloom_check(counting_bloom_t *bloom, const char *s, size_t len)
{
    ...
    hash_func(bloom, s, len, hashes); //by xp: calculate the corresponding hash value by each hash function for element s
